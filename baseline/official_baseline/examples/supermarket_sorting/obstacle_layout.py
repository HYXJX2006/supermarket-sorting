...
OBSTACLE_GS_PLY = "shentoon/yellowbox_obstacle_fit.ply"
...
def _grid_path_length(selected):
    radius = ROBOT_CLEARANCE_RADIUS
...
def evaluate_obstacle_layout(selected):
    ...
    path_length = _grid_path_length(selected)
    if path_length is None:
        return LayoutEvaluation(False, "no robot-sized route from shelf to delivery",
                                occupied_bands=occupied_bands)
    detour = path_length - EMPTY_CORRIDOR_PATH_LENGTH
