...
from obstacle_layout import OBSTACLE_BODIES, OBSTACLE_GS_PLY, generate_obstacle_layout
...
DEFAULT_TASKS = "all"
FIXED_BASELINE_TARGET = "product_032"
...
def write_runtime_xml(
    ...
    if obstacle_yaw_overrides:
        ...
        for body_name, yaw in obstacle_yaw_overrides.items():
            ...
            if n != 1:
                raise RuntimeError(
                    f"randomize: expected exactly 1 obstacle body yaw for {body_name}, got {n}")
...
def fixed_baseline_layout(layout, target_body=FIXED_BASELINE_TARGET):
    """Keep the baseline target centered and remove other colas from its row."""
    target_i = next(
        (i for i, slot in enumerate(layout) if slot["body"] == target_body), None
    )
    if target_i is None:
        raise RuntimeError(f"fixed baseline target not found: {target_body}")
    target = layout[target_i]
    if target["object_kind"] != "kele" or target["column"] != "C2":
        raise RuntimeError(f"fixed baseline target is not the centered cola: {target}")

    row_key = (target["shelf"], target["level"])
    extra_kele = [
        i for i, slot in enumerate(layout)
        if (slot["shelf"], slot["level"]) == row_key
        and slot["object_kind"] == "kele"
        and i != target_i
    ]
    donors = [
        i for i, slot in enumerate(layout)
        if slot["level"] == target["level"]
        and slot["shelf"] != target["shelf"]
        and slot["object_kind"] != "kele"
    ]
    if len(donors) < len(extra_kele):
        raise RuntimeError("not enough non-cola donor slots for fixed baseline")

    destination = {}
    for extra_i, donor_i in zip(extra_kele, donors):
        destination[extra_i] = donor_i
        destination[donor_i] = extra_i

    new_layout = [slot.copy() for slot in layout]
    pos_overrides = {}
    for body_i, slot_i in destination.items():
        source = layout[body_i]
        slot = layout[slot_i]
        half_height = (
            source["world_position"][2] - SHELF_SURFACE[source["level"]]
        )
        x, y, _ = slot["world_position"]
        z = SHELF_SURFACE[slot["level"]] + half_height
        for key in ("shelf", "level", "column", "aruco_id"):
            new_layout[body_i][key] = slot[key]
        new_layout[body_i]["world_position"] = [x, y, z]
        pos_overrides[source["body"]] = (x, y, z)

    row_kele = [
        slot for slot in new_layout
        if (slot["shelf"], slot["level"]) == row_key
        and slot["object_kind"] == "kele"
    ]
    if [slot["body"] for slot in row_kele] != [target_body]:
        raise RuntimeError(f"fixed baseline row still has extra colas: {row_kele}")
    return new_layout, pos_overrides
...
def build_config():
    ...
    pos_overrides = {}
    fixed_baseline = env_flag("SUPERMARKET_FIXED_BASELINE", False)
    randomize_products = env_flag("SUPERMARKET_RANDOMIZE", True)
    if fixed_baseline:
        if randomize_products:
            raise ValueError(
                "SUPERMARKET_FIXED_BASELINE=1 requires SUPERMARKET_RANDOMIZE=0"
            )
        layout, pos_overrides = fixed_baseline_layout(layout)
        print(
            "[server] fixed baseline layout "
            f"(target={FIXED_BASELINE_TARGET}, shelf D/L2 has one cola)"
        )
    elif randomize_products:
    ...
    if env_flag("SUPERMARKET_RANDOMIZE_OBSTACLES", True):
        ...
        obstacle_layout = generate_obstacle_layout(obstacle_seed)
    ...
    for body_name in OBSTACLE_BODIES:
        cfg.gs_model_dict[body_name] = OBSTACLE_GS_PLY
