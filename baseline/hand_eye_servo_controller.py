...
class HandEyeServo(Node):
    ...
    def run(self) -> int:
        ...
        for iteration in range(self.max_iterations):
            ...
            # 半量迭代 2-3 次收敛到 ±1cm。
            # 有效增益必须 <1.0 才收敛。2026-09-12 夜实测（C/L2/C3 一轮）：
            # 三次测量 4.3→5.8→7.6cm，每次只让误差涨 1.4~1.8cm，即真实灵敏度
            # 约 0.28 m/rad（昨天标定的 0.4 是另一批角度下测的），而原式按 0.4
            # 计算再乘 1.25（0.5 阻尼的倒数）→ 等效增益 1.25 > 1，越修越偏，
            # 第三次直接撞到 ±0.08 限幅。改为 1/0.28*0.45≈1.6 的保守半量：
            # 等效增益 ~0.6，宁可多迭代两次也不要振荡。
            SERVO_LATERAL_GAIN = 1.6
            dj3 = max(-0.08, min(0.08, -err_lateral * SERVO_LATERAL_GAIN))
            sens = -0.4
            if abs(dj3) < 1e-3:
                continue
